...
* "Да" для включения звуковых сигналов об ошибках, независимо от вашей текущей версии NVDA.
...
Сообщение следующим образом будет записано в журнал на уровне информации:
`-- NDTT marker 0 --`
...
* Строка заголовка записанного в журнал сообщения содержит функцию, которая зажурналировала это сообщение, например:
...
В консоли вы можете вызвать следующую функцию для просмотра исходного кода, который определяет переменная `myVar`:
`openCodeFile(myVar)`
...
* Новая команда позволяет размещать маркеры в журнале; и конкретные команды быстрой навигации в режиме чтения журнала позволяют переходить к ним.
